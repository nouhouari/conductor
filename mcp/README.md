...
Projects generated by `init_project` depend on `com.nouhouari.conductor:conductor-core`, published
to GitHub Packages. Maven requires authentication for GitHub Packages even on public repositories,
so the generated `pom.xml` declares a `github-conductor` repository and the generated `README.md`
explains the matching `~/.m2/settings.xml` `<server>` entry (a token with the `read:packages`
scope). Developers working from a monorepo checkout can instead run `cd java && mvn install` and
